...
$Computers  = Get-ADComputer -Filter { enabled -eq $true } -Properties msLaps-EncryptedPassword,OperatingSystem,LastLogonTimeStamp,msLAPS-PasswordExpirationTime
...
foreach ($cptr in $Computers)
{
    ...
    if ($cptr.'msLaps-EncryptedPassword' -or $cptr.'msLAPS-PasswordExpirationTime') 
    {
        $LapsSet = $true
...
ConvertTo-Html -Body $reportHtml -Head $Header | Out-File $PSScriptRoot\LAPS-Report_$(get-date -f yyyy-MM-dd_HH-mm-ss).html -Force
